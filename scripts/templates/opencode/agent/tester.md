...
2. **Optional pre-step**: if CodeGraph is installed (`Test-Path .codegraph/codegraph.db` / `ls .codegraph/codegraph.db`), use `node .codegraph/node_modules/.bin/codegraph affected <file1> <file2> ...` to identify the subset of tests likely affected by the Programmer's changes. Run that subset first; full suite second. This avoids running the entire test suite when a 2-file change only touches one module. (CodeGraph se instala aislado en `.codegraph/` — el binario no está en `node_modules/` del proyecto principal, por eso el path explícito.)
...
If `node .codegraph/node_modules/.bin/codegraph affected <changed_files>` returns a non-empty list:
