...
// Crea el shim si no existe. Es un archivo CommonJS que carga dinámicamente
// el entry point real de CodeGraph leyendo su package.json — así si CodeGraph
// cambia el path interno entre versiones (dist/cli.js → src/index.js, etc),
// el shim sigue funcionando sin tocar.
async function ensureCodeGraphShim() {
  if (await fileExists(CODEGRAPH_SHIM)) return { created: false };

  const shim = `// Stable invocation shim for CodeGraph — generated by Phobos wizard.
// Bypasses pnpm/npm/yarn .bin wrapper inconsistencies by loading the actual
// JS entry point directly via require(). Re-generated on each install run.
const { join } = require('node:path');

const pkgPath = join(__dirname, 'node_modules', '@colbymchenry', 'codegraph', 'package.json');
...
require(join(__dirname, 'node_modules', '@colbymchenry', 'codegraph', entry));
`;
